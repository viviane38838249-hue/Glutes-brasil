...
const etapas=[{icone:"🧠",titulo:"ANALISANDO SUAS RESPOSTAS...",subtitulo:"Processando seu perfil personalizado"},{icone:"📅",titulo:"PREPARANDO OS 28 DIAS...",subtitulo:"Criando seu plano de treino exclusivo"},{icone:"🎯",titulo:"CALCULANDO SEU POTENCIAL...",subtitulo:"Identificando seu objetivo de bumbum ideal"},{icone:"📲",titulo:"QUASE PRONTO!",subtitulo:"Preparando seu treino personalizado"}];const DURACAO_ETAPA=1800;const TOTAL_ETAPAS=etapas.length;function atualizarChecklist(numeroEtapa){checklistItems.forEach(function(item,index){item.classList.remove('is-done','is-active','is-pending');if(index<numeroEtapa){item.classList.add('is-done');}else if(index===numeroEtapa){item.classList.add('is-active');}else{item.classList.add('is-pending');}});}
